...
public class rendezvouzService implements IService<rendezvouz> {
    ...
    public List<rendezvouz> ffxd(String ff) throws SQLException {
        String sql = "SELECT * FROM rendezvouz WHERE email LIKE CONCAT('%', ?, '%') OR daterdv LIKE CONCAT('%', ?, '%')";
        PreparedStatement statement = connection.prepareStatement(sql);
        statement.setString(1, ff);
        statement.setString(2, ff);
        ResultSet rs = statement.executeQuery();
        List<rendezvouz> people = new ArrayList<>();
        while (rs.next()) {
            rendezvouz p = new rendezvouz();
            p.setId(rs.getInt("id"));
            p.setEmail(rs.getString("email"));
            p.setDaterdv(Timestamp.valueOf(rs.getString("daterdv")));
