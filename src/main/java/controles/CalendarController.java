            ...
            for (int j = 0; j < 7; j++) {
                ...
                if (calculatedDate > dateOffset) {
                    ...
                    if (currentDate <= monthMaxDate) {
                        ...
                        if (!rdvs.isEmpty()) {
                            Text rendezvouzText = new Text();
                            for (rendezvouz rdv : rdvs) {
                                rendezvouzText.setText(rendezvouzText.getText() + rdv.getEmail() + " - " + rdv.getDaterdv() + "\n");
                            }
                            rendezvouzText.setStyle("-fx-font-weight: bold"); // Make the text bold
                            double rendezvouzTextTranslationY = (rectangleHeight / 2) * 0.75;
                            rendezvouzText.setTranslateY(rendezvouzTextTranslationY);
                            stackPane.getChildren().add(rendezvouzText);
