...
public class Listeloc {
    ...
    void initialize() throws SQLException, IOException {
      ...
      //  table.getColumns().addAll(nomColumn, adresseColumn);
       // table.setItems(list);
        ...
        /* search.textProperty().addListener((observable, oldValue, newValue) -> {
            filteredData.setPredicate(local -> {
                ...
                if (local.getNom().toLowerCase().contains(searchKeyword)) {
                    return true;
                } else if (local.getAdresse().toLowerCase().contains(searchKeyword)) {
                    return true;
                } else
                    return false;
            });
    ...
    void onajouter(ActionEvent event) throws IOException {
       /* if (nom.getText().isEmpty() || adresse.getText().isEmpty()) {
            // Show an alert indicating that fields cannot be empty
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Warning");
            alert.setHeaderText(null);
            alert.setContentText("Nom and Adresse cannot be empty!");
            alert.showAndWait();
        } else {
