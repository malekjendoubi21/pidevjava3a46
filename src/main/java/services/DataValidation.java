...
public class DataValidation {
    ...
    public static boolean dateIsValid(TextField inputTextField, Label inputLabel, String validationText) {
        boolean isValid = true;
        String validationString = null;

        try {
            LocalDate date = LocalDate.parse(inputTextField.getText());
            if (date.isBefore(LocalDate.now())) {
                isValid = false;
                validationString = "La date ne peut pas être antérieure à aujourd'hui.";
            }
        } catch (Exception e) {
            isValid = false;
            validationString = validationText;
