...
public class OrganisationService implements IService<Organisation> {
    ...
    public List<Organisation> rech(String dd) throws SQLException {

        String sql = "SELECT * FROM Organisation WHERE nom LIKE CONCAT('%', ?, '%') OR email LIKE CONCAT('%', ?, '%') OR adresse LIKE CONCAT('%', ?, '%') OR num_tel LIKE CONCAT('%', ?, '%')";
